...
def compare_heat_transfer_modes(T_s, T_inf, k=1.0, L=0.1, h=10.0, eps=0.9):
    """
    Compare heat flux from conduction, convection, and radiation.
    All temperatures in Kelvin. Returns heat flux in W/m².
    """
    q_cond = k * (T_s - T_inf) / L
    q_conv = h * (T_s - T_inf)
    q_rad = eps * SIGMA * (T_s**4 - T_inf**4)
    return {'conduction': q_cond, 'convection': q_conv, 'radiation': q_rad}
...
result = compare_heat_transfer_modes(T_s=T_s, T_inf=T_inf, h=h)

print(f"Surface: {T_s} K, Ambient: {T_inf} K")
print(f"\nHeat flux comparison (W/m²):")
print(f"  Conduction: {result['conduction']:,.0f}")
print(f"  Convection: {result['convection']:,.0f}")
print(f"  Radiation:  {result['radiation']:,.0f}")
...
|--------|------|----------|-------------|
| $q$ or $Q$ | Heat transfer rate | W = J/s | Total power transferred |
